...
class DialogService{
    ...
    createLabeledInput = (target, title, inputType, value) => {
    ...
    createListInput = (label, inputList, id) => {
    ...
    buildPopUpDialog = (recipe) =>
    {
        ...
        // create hidden, uneditable input...
        if(recipe){
        ...
        if(recipe) idField.value = recipe._id;
    ...
    handleSave = (e) => {
        ...
        if(recipeId.value !== ''){
            this.browserService.updateRecipe({
                _id: recipeId,
                name: recipeName,
                grains: allGrains,
                hops: allHops,
